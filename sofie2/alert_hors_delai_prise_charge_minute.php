...
{	
	foreach($rows as $row)
		...
		echo $kannel_sendsms_uri .= "username=modem1&password=458asn&smsc=modem1&from=".$sofie_SMS_SHORTCODE."&to=+".substr($sofie_NUMERO,-8)."&text=".urlencode($sms_content.": PRISE DE COMMANDE HORS DELAI!");
		...
		if($send_state="status=0"){
			...
			$UpdateQuery2 = "INSERT INTO `t_notification` (DateHeureNotif,MotifNotif,IDPanne,IDNumAppel)VALUES(NOW(),6,f_getPanneIdByPanneTicket(".$row->NumPanne."),f_getIDNumAppelAgentByIDAgent(".$row->IDAgent."))";
